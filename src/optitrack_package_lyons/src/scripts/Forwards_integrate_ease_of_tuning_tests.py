...
#basically the same as the usual forwards integrate but also resets the initial position and does not change the s_dot of obstacle
...
safety_value = 0
safety_value_to_publish = 0
def safety_value_subscriber_callback(msg):
    # need global keyword to overwrite global variable
    global safety_value, safety_value_to_publish
    #this will set it to true once you press enter and stop checking
    if msg.data == 1:
        safety_value_to_publish = 1
    safety_value = safety_value_to_publish
...
def forwards_integrate():
    ...
    simulation_time_publisher = rospy.Publisher('simulation_time', Float32, queue_size = 10)
    ...
    s_dot_dyn_obst_selection = [0.9, 0.9]
    ...
    # initiate states
    state_3_reset = [Checkpoints_x[0]+0.1, Checkpoints_y[0], 0, 1]
    ...
    previous_index = 0
    simulation_time = 0
    reset_simulation_time = 60  # decide when to reset
    dt = 0.1 # should match the value in MPCC ease of tuning tests
    failed_overtakes_count = 0
    while not rospy.is_shutdown():
        ...
        if safety_value == 0:
            ...
            for ii in range(N):
                s_dyn_pred_i = s_0_dyn_obst + s_dot_dyn_obst * dt_int * ii
                ...
                dyn_ob_traj_r[
                    ii] = r_0_dyn_obst + r_enlarge_rate_dyn_obst * dt_int * ii  # enalrge radius to simulate uncertainty
        ...
        if simulation_time > reset_simulation_time or state_3[0] - dyn_ob_traj_x[0] > 0.1:
            print('time remaining when reset = ',-simulation_time+reset_simulation_time)
            # state_3 = state_3_reset
            # s_0_dyn_obst = s_0_dyn_obst_reset
            s_0_dyn_obst = s_0_dyn_obst_reset  # reset in front of the ego vehicle at a given position
            ...
            #also reset position of vehicle
            state_3 = state_3_reset

            simulation_time = 0 #reset time

        else:
            simulation_time = simulation_time + dt_int * safety_value
        ...
        simulation_time_publisher.publish(simulation_time)
