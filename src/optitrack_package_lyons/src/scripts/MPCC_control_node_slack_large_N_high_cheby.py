...
dyn_ob_traj_x = np.zeros(30)+10
dyn_ob_traj_y = np.zeros(30)+10
...
def occupancy_xyr_4_subscriber_callback(msg):
    ...
    if len(dyn_ob_traj) == 45:
        dyn_ob_traj_x[0:15] = dyn_ob_traj[0:15]
        dyn_ob_traj_y[0:15] = dyn_ob_traj[15:30]
        dyn_ob_traj_r[0:15] = dyn_ob_traj[30:45]
        dyn_ob_traj_x[15:31] = dyn_ob_traj[14] + np.zeros(15)
        dyn_ob_traj_y[15:31] = dyn_ob_traj[29] + np.zeros(15)
        dyn_ob_traj_r[15:31] = dyn_ob_traj[34] + np.zeros(15)
    elif len(dyn_ob_traj) == 90:
        dyn_ob_traj_x[0:31] = dyn_ob_traj[0:30]
        dyn_ob_traj_y[0:31] = dyn_ob_traj[30:60]
        dyn_ob_traj_r[0:31] = dyn_ob_traj[60:90]
...
def MPCC_controller():
    ...
    if solver_choice == 1:
        ...
        solver_name = 'Machining_slack__TerminalCost_high_order_cheby'
        ...
        solver_name = 'Cheby_large_DS_slack_TerminalCost_high_order_cheby'
    ...
    if solver_choice == 1 or solver_choice == 3 or solver_choice == 4:
        ...
        q3 = 0.1  # relative weight of steering (Weighting the two inputs identically to be fair
                  # vs machining formulation)
        #low weights
        # q2 = 0.3  # relative weight of lat error
        # # in the lab it is best to use higher steering weights (like =1)
        # q3 = 0.1  # relative weight of steering (Weighting the two inputs identically to be fair
        #           # vs machining formulation)
        q4 = 1  # relative weight of throttle
        ...
        # #high weights
        q2 = 0.5  # relative weight of lat error
        q3 = 5  # relative weight of lag error -- this is the extra term that needs to be tuned
        #low weights
        # q2 = 0.3  # relative weight of lat error
        # q3 = 0.6  # relative weight of lag error -- this is the extra term that needs to be tuned
        # weights
        # q2 = 1  # relative weight of lat error
        # q3 = 5  # relative weight of lag error -- this is the extra term that needs to be tuned
        q4 = 1  # relative weight of inputs  (weighted the same here)
    ...
    while not rospy.is_shutdown():
        ...
        coeffx, coeffy = evaluate_local_path_Chebyshev_coefficients_high_order_cheby(s, x_of_s, y_of_s, Ds_forecast, s_vals_global_path, loop_path,
                                                          Cheby_data_points)
        # stop_clock_time_cheby_fitting = rospy.get_rostime()
        # total_time_cheby_fitting = stop_clock_time_cheby_fitting.secs - start_clock_time_cheby_fitting.secs + (
        #             stop_clock_time_cheby_fitting.nsecs - start_clock_time_cheby_fitting.nsecs) / 1000000000
        # print('Fitting cheby time = ', total_time_cheby_fitting)
        ...
        if warm_start:
            ...
            # dummy_x = [0,0,0,0,0]
            # x0_array = np.concatenate(([0, 0, 0], xinit), axis=0)
            # for ii in range(1, N):
            #     dummy_x[4] = dummy_x[4] + dt * V_target
            #     Cx_out, Cy_out, s_dot, k, x_Cdev, y_Cdev, x_Cdev2, y_Cdev2 = evaluate_spline_quantities_cheby_high_order(dummy_x, params_i)
            #     x0_i = [0, 0, 0, Cx_out, Cy_out, 0, V_target, dummy_x[4]]
            #     x0_array = [*x0_array, *x0_i]
        ...
        max_steer_right = 18
        max_steer_left = 15
        problem["lb"] = np.tile([0.10, -max_steer_right / 180.0 * np.pi, 0, a], N)
        problem["ub"] = np.tile([0.15, max_steer_left / 180.0 * np.pi, 100, b], N)

        #ok this is also pretty quick up to here less than 0.002 seconds so ok
        # stop_clock_time_before_solver = rospy.get_rostime()
        # total_time_before_solver = stop_clock_time_before_solver.secs-start_clock_time.secs+(stop_clock_time_before_solver.nsecs-start_clock_time.nsecs)/1000000000
        # print('Time before the solver is called = ',total_time_before_solver)
        ...
        if exitflag != 1:
            ...
            if warm_start == True:
                ...
                #this is not a good idea because if you are in a difficoult spot and youo don't converge,
                # and you try to solve the problem twice you just get too much latency to do anything.
        ...
        # print(-output_array[0, 1])
        if -output_array[0, 1] < 0:
            steering_val = Float32(-output_array[0, 1] / (max_steer_left / 180.0 * np.pi))
        else:
            steering_val = Float32(-output_array[0, 1] / (max_steer_right / 180.0 * np.pi))
        ...
        ##
        velocity_stacked = [*velocity_stacked, output_array[1, 6]]



        # ok this is also really fast less than 0.001 seconds
        # stop_clock_after_solver = rospy.get_rostime()
        # total_time_after_solver = stop_clock_after_solver.secs-start_clock_time_after_solver.secs+(stop_clock_after_solver.nsecs-start_clock_time_after_solver.nsecs)/1000000000
        # print('Time after the solver is called = ', total_time_after_solver)
        ...
        if plot_figures and plot_counter == plot_every_n_figures:
            ...
            for ii in range(0, N):
                ...
                Cx_out, Cy_out, s_dot, k, x_Cdev, y_Cdev, x_Cdev2, y_Cdev2 = evaluate_spline_quantities_cheby_high_order(state_i,
                                                                                                              params_i)
                # norm = np.sqrt(x_Cdev ** 2 + y_Cdev ** 2)
                # # chek if curvature boundarie applies
                # V_x_left = (-l_shift + l_width / 2) * x_Cdev / norm
    ...
    plt.figure(3)
    plt.plot(velocity_stacked, label="velocity ")
    plt.plot(np.ones(len(velocity_stacked))*V_target, label="velocity target")
    plt.show()

    # plot full trajectory
    xy_trajectory = np.array([0 , 0])
    for i in range(10, len(solver_time_stacked)-10):
        xy_trajectory = np.vstack([xy_trajectory, output_array_stacked[N*(i), 3:5]])

    plt.figure(4)
    plt.plot(x_vals_original_path, y_vals_original_path, label="Original global path")
    plt.plot(xy_trajectory[:, 0], xy_trajectory[:, 1], label="trajectory of vehicle")
    ...
    #get time and date
    # filename = '/home/lorenzo/OneDrive/PhD/ubuntu_stuff/python_world/MPCC_Lyons/Racetrack_tests/' + solver_name + date + '_' + time + '.db'
    filename = '/home/lorenzo/OneDrive/PhD/ubuntu_stuff/python_world/MPCC_Lyons/Racetrack_tests/AA_racetrack_video' + solver_name + date + '_' + time + '.db'
