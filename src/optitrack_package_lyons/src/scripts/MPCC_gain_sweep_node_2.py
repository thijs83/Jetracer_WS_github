...
#sweeping indexes
n_trials = 20  # number of trials for each q2 - q3 combination
...
q2_gain_sweep_vec = np.linspace(0, 5, 11)
q2_gain_sweep_vec[0] = 0.0001

q2_indexes = range(len(q2_gain_sweep_vec))
if solver_choice == 2:
    q3_gain_sweep_vec = np.linspace(0, 10, 11)
...
dyn_ob_traj_x = np.zeros(30)
dyn_ob_traj_y = np.zeros(30)
dyn_ob_traj_r = np.zeros(30)
...
#set dynamic obstacle resetting parameters
s_0_dyn_obst_reset = 2
s_0_dyn_obst = s_0_dyn_obst_reset
s_dot_dyn_obst_base = 0.2
r_enlarge_rate_dyn_obst_base = 0.01
r_0_dyn_obst_base = 0.10
# s_dot_dyn_obst = 0.2
# r_0_dyn_obst = 0.10
...
for k in range(len(q2_gain_sweep_vec)):
    ...
    for i in range(len(q3_gain_sweep_vec)):
        ...
        for trial in range(0, n_trials):
            ...
            state = [Checkpoints_x[initial_checkpoint], Checkpoints_y[initial_checkpoint], 0, 0.75]
            #reset obstacle
            s_0_dyn_obst = s_0_dyn_obst_reset + 0.1 * (random.randint(-100, +100) / 100)  # reset in front of the ego vehicle
            # reset properties
            r_0_dyn_obst = r_0_dyn_obst_base + 0.05 * (random.randint(-100, +100) / 100)
            r_enlarge_rate_dyn_obst = r_enlarge_rate_dyn_obst_base + 0.01 * (random.randint(-100, +100) / 100)
            s_0_dyn_obst = s_0_dyn_obst + 0.1 * (random.randint(-100, +100) / 100)

            previous_index = initial_checkpoint
            warm_start_solution_0 = np.concatenate(([0, 0, 0], [0, 0, 0, 0, 0]), axis=0)
            warm_start_solution = np.tile(warm_start_solution_0, N)

            #set initial obstacle values
            r_0_dyn_obst = r_0_dyn_obst_base + 0.05 * (random.randint(-100, +100) / 100)
            r_enlarge_rate_dyn_obst = r_enlarge_rate_dyn_obst_base + 0.01 * (random.randint(-100, +100) / 100)
            s_dot_dyn_obst = s_dot_dyn_obst_base + 0.1 * (random.randint(-100, +100) / 100)
            ...
            for t in range(0, time_limit):
                ...
                for ii in range(N):
                    s_dyn_pred_i = s_0_dyn_obst + s_dot_dyn_obst * dt * ii
                ...
                if s - s_0_dyn_obst > 0.5:
                    s_0_dyn_obst = s + s_0_dyn_obst_reset
                    # reset properties
                    r_0_dyn_obst = r_0_dyn_obst_base + 0.05 * (random.randint(-100, +100) / 100)
                    r_enlarge_rate_dyn_obst = r_enlarge_rate_dyn_obst_base + 0.01 * (random.randint(-100, +100) / 100)
                    s_dot_dyn_obst = s_dot_dyn_obst_base + 0.1 * (random.randint(-100, +100) / 100)
